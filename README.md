...
└── .github/workflows/ci.yml # CI: install deps, lint with ruff, run pytest
```
...
`test_agents.py` covers agent logic that doesn't require live credentials or
network calls that can't be sandboxed (joke generation, weather error
handling, calendar event creation/validation).

## Continuous Integration

`.github/workflows/ci.yml` runs on every push/PR to `main`: installs
`requirements-dev.txt`, lints with `ruff check .`, and runs `pytest -v`.
